...
public class CurrencyController : MonoBehaviour
{
  ...
  void Start()
  {
    if (Game.StateManager.CurrentState == GameState.Menu)
    {
      ...
      if (Game.Settings.SessionMoney > 0)
      {
        Game.Settings.SoftCurrency = Game.Settings.SoftCurrency + Game.Settings.SessionMoney;
  ...
  private bool IsTimeToUpdate
  {
    get
    {
      ...
      if (Game.StateManager.CurrentState == GameState.Menu)
      {
        if (CurrentAmount < Game.Settings.SoftCurrency)
        {
          return true;
        }
      }
